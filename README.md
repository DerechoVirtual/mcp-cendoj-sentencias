# Jurisprudenciator — Servidor MCP de jurisprudencia

Busca y **lee sentencias** de las **fuentes oficiales y gratuitas** de
jurisprudencia española directamente desde **Claude** (Desktop / Cowork).
Encuentra la **mejor** sentencia para un caso, extrae los **párrafos exactos**
con su **ECLI**, y trabaja a **gran velocidad**.

> **⚡ Rápido de verdad:** la mejor sentencia con sus párrafos en **~2-3 s**, y
> **30 sentencias con sus párrafos clave en ~6 s**. El control antidescargas de
> la fuente es **por sesión** (no por IP): el servidor reparte las descargas
> entre varias sesiones frescas (**multi-sesión**) y **esquiva las
> comprobaciones sin pausas ni intervención del usuario**. Sin API keys, sin coste.

## Qué puede hacer

- **Buscar** jurisprudencia con todos los filtros de la fuente: base TS/AN, fechas,
...
Juzgados…). La fuente es pública: **no hay login ni usuario**.
...
| Variable | Para qué |
|---|---|
| `DOWNLOAD_DIR` | Dónde guardar PDFs/textos **si pides guardarlos**. Def.: `…\Documents\sentencias`. |
...
2. **`leer_sentencias(parrafos=N)`** → lee en memoria y devuelve solo los **N
   ...
   **No guarda nada** salvo que lo pidas. Esquiva las comprobaciones solo.
...
- **Multi-sesión**: la comprobación antidescargas es **por sesión** y salta sobre
  la 6ª-7ª descarga. El motor usa **varias sesiones frescas en paralelo** (5
  descargas cada una) → casi nunca aparece, y si aparece se **esquiva** abriendo
  otra sesión. Sin pausas, sin resolver nada a mano.
...
jurisprudencia). El multi-sesión reparte la carga, pero no martillees la fuente con
...
- Por defecto **no guarda nada** en tu disco (lee en memoria). Solo lee/consulta la
  fuente oficial; no envía nada a terceros.
