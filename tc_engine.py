...
_ses: dict = {"cliente": None, "token": "", "ts": 0.0}
...
def _con_sesion():
    if _ses["cliente"] is None or time.time() - _ses["ts"] > _SES_TTL:
        _ses["cliente"], _ses["token"] = _sesion_fresca()
        _ses["ts"] = time.time()
    return _ses["cliente"], _ses["token"]


def _buscar_form(form: dict, maximo: int) -> tuple[list[dict], str]:
    """POST del formulario + GET de la lista, bajo lock (los resultados viven en
    la sesion del servidor). Devuelve (docs, total)."""
    with _lock:
        for intento in range(2):
            try:
                ...
                if r.status_code != 302 or "Resolucion/List" not in loc:
                    if intento == 0:
                        _ses["cliente"] = None  # token/cookie caducados: renovar
                        continue
                    return [], "0"
                docs: list[dict] = []
                total = "?"
                ...
                # ignora): se recorren paginas hasta reunir 'maximo' o agotarse.
                while len(docs) < maximo and pagina <= 12:
                    rl = c.get(f"{BASE}/es/Resolucion/List",
                               params={"sortOrder": "desc", "page": pagina})
                    if rl.status_code != 200:
                        break
                    if pagina == 1:
                        mt = re.search(r"Se han encontrado\s*<b>([\d.]+)</b>", rl.text)
                        total = mt.group(1) if mt else "?"
                    nuevos = _parse_lista(rl.text)
                    if not nuevos:
                        break
                    docs.extend(nuevos)
                    if total not in ("?", "") and len(docs) >= int(total.replace(".", "")):
                        break
                    pagina += 1
                if docs:
                    docs[0]["_total"] = total
...
def _parse_lista(html: str) -> list[dict]:
    ...
    for t in trozos[1:]:
        ...
        docs.append({
            ...
            "resumen": resumen, "recurso": recurso,
...
def buscar_docs(consulta: str, fecha_desde: str = "", fecha_hasta: str = "",
    ...
    para el ModelState: si no se dan, rango completo 1980-hoy)."""
    hasta_def = time.strftime("31/12/%Y")
    form = {"BUSQUEDA_LIBRE": (consulta or "").strip(),
            "TIPO_BUSQUEDA_LIBRE": "0",
    ...
    docs, _ = _buscar_form(form, max(1, int(maximo)))
    return docs
...
def leer_doc(d: dict, parrafos: int = 0, terminos: str = "", max_chars: int = 0):
    ...
    try:
        r = _cliente_lector().get(f"{BASE}/es/Resolucion/Show/{id_hj}")
    except httpx.TransportError as e:
        _lector["c"] = None
        return None, f"red: {e}"
    if r.status_code != 200:
        return None, f"HTTP {r.status_code} del Tribunal Constitucional"
    texto, meta = _texto_ficha(r.text)
    if len(texto) < 300:
