# -*- coding: utf-8 -*-
"""Banco BURGOS (bopbur Drupal + índice empaquetado, familia «burgos»).
...
Los casos son normas REALES vistas en el índice (títulos del BOP 2015-2026).
Uso: python -X utf8 _test_bop_burgos.py
"""
import os
import re
import statistics
...
ENCONTRAR = [
    ("Burgos", "terrazas hostelería", r"terraza"),
    ("Burgos", "movilidad sostenible", r"movilidad"),
    ("Burgos", "locales de ocio privado", r"ocio"),
    ("Burgos", "cementerio", r"cementerio"),
    ("Miranda de Ebro", "bienestar animal", r"animal"),
    ("Miranda de Ebro", "impuesto sobre bienes inmuebles", r"bienes inmuebles"),
    ("Miranda de Ebro", "alcantarillado vertidos", r"alcantarillado|vertido"),
    ("Miranda de Ebro", "mercadillo", r"mercadillo|venta ambulante"),
    ("Aranda de Duero", "terrazas veladores", r"terraza|velador"),
    ("Aranda de Duero", "venta ambulante", r"ambulante"),
    ("Aranda de Duero", "impuesto sobre bienes inmuebles", r"bienes inmuebles"),
    ("Briviesca", "suministro de agua", r"agua"),
    ("Medina de Pomar", "prestación patrimonial", r"prestaci"),
    ("Villarcayo", "impuesto sobre bienes inmuebles", r"bienes inmuebles|\bIBI\b"),
    ("Villarcayo", "limpieza y vallado de solares", r"solares|vallado|ornato"),
    ("Lerma", "agua potable", r"agua"),
    ("Salas de los Infantes", "vertidos alcantarillado", r"vertido|alcantarillado"),
    ("Salas de los Infantes", "depuración de aguas", r"depuraci|agua"),
]
# materias que NO existen en ese ayuntamiento: el motor debe responder honesto
HONESTO = [
    ("Lerma", "puerto deportivo"),
    ("Briviesca", "aeropuerto"),
]
...
def main():
    for muni in ("Burgos", "Miranda de Ebro", "Aranda de Duero"):
        p = B.provincia_de(muni)
        print(f"enrutado {muni:22} -> {p}" + ("" if p == "burgos" else "   <-- MAL"))
    ok = tot = 0
    tiempos = []
    print("\n== BURGOS debe-encontrar (buscar + leer parrafos=3) ==")
    ...
    print(f"\nRESULTADO BURGOS: {ok}/{tot} OK · mediana {med:.2f} s · máximo {mx:.2f} s"
